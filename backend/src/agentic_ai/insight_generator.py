"""Generate insights using Bedrock with fallback logic."""
...
class InsightGenerator:
    ...
    def generate_insight(self, analysis_output_path: str | Path = ANALYSIS_PATH,
                         analysis_data: Dict[str, Any] | None = None,
                         append_to_file: bool = True) -> str:
        """Generate insight and optionally append to file."""
        if not analysis_data and analysis_output_path:
            try:
                ...
                logger.error("Failed to read analysis file: %s", e)
                return ""
...
"""
        ...
        try:
            logger.info("Invoking Bedrock for insight generation...")
            ...
            logger.warning("Bedrock failed (%s), using local fallback...", e)
        ...
        if append_to_file:
            try:
                ...
                logger.error("Failed to append insight to file: %s", e)
