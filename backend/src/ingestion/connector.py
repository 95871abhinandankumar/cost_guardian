...
class AWSConnector:
    ...
    def load_mock_file(self, file_path=None):
        """
        Load local mock JSONL file.
        If file_path is None, default to mock_data.jsonl in the ingestion folder.
        """
        if file_path is None:
            file_path = os.path.join(os.path.dirname(__file__), "mock_data.jsonl")
    ...
    def get_mock_data_for_tenant(self, tenant_id):
        ...
        if not tenant_data:
            tenant_data = all_data  # fallback to all mock data for testing
        return tenant_data
